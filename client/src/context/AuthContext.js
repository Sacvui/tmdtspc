...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const autoLogin = () => {
      ...
      if (savedUser) {
        ...
        if (currentUser) {
          ...
          setUser(updatedUser);
          console.log('✅ Tự động đăng nhập lại:', updatedUser.name || updatedUser.phone || updatedUser.username);
        } else {
    ...
    // Listen for storage changes (when login from another tab)
